...
f1 = np.zeros(cutoff, dtype=complex); f1[1] = 1.0
f2 = np.zeros(cutoff, dtype=complex); f2[1] = 1.0
...
# vacuum fock vectors
vac = np.zeros(cutoff, dtype=complex); vac[0] = 1.0
...
print(f"Numeric p(0) = {p0:.8f}, analytic = {analytic:.8f}, error = {abs(p0-analytic):.2e}")
...
rho_cond, Pwin, joint = composer.compose_pair_cached(vac, vac, pA=1.0, pB=1.0, homodyne_x=0.0, homodyne_window=0.4)
print(f"P(window=0.4 around 0) = {Pwin:.6f}")

plt.plot(xs, p_xs); plt.xlabel('x'); plt.ylabel('p(x)'); plt.title('Vacuum homodyne density (window integration)'); plt.show()
...
circ = GaussianHeraldCircuit(n_signal=1, n_control=1, tmss_squeezing=[0.8], us_params=None, uc_params=None)
circ.build()
...
rho_cond, p_x_or_Pwin, joint = composer.compose_pair_cached(state_vec.flatten(), state_vec.flatten(), pA=p_herald, pB=p_herald, homodyne_x=0.0, homodyne_window=0.2)
print("joint probability of two-block composition (includes both heralds and homodyne):", joint)
plot_distribution(np.real(np.diag(rho_cond)), title='Final photon distribution after compose_pair')
...
final_rho, final_joint = topo.evaluate_topology(composer, focks, ps, homodyne_x=0.0, homodyne_window=0.2)
print("final joint probability for 4-block full-binary topology:", final_joint)
plot_distribution(np.real(np.diag(final_rho)), title='Final photon number distribution (4-block tree)')
...
vac = np.zeros(composer.cutoff, dtype=complex); vac[0] = 1.0

# run same compose_pair_cached twice and measure time
import time
start = time.time()
rho1, p1, joint1 = composer.compose_pair_cached(vac, vac, pA=0.5, pB=0.5, homodyne_x=0.0, homodyne_window=0.2)
cold = time.time() - start
start = time.time()
rho2, p2, joint2 = composer.compose_pair_cached(vac, vac, pA=0.5, pB=0.5, homodyne_x=0.0, homodyne_window=0.2)
